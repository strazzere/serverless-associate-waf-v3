...
const AssociateWafPlugin = require('./index')
...
describe('AssociateWafPlugin', () => {
  ...
  beforeEach(() => {
    ...
    awsProvider.getAccountInfo = async () => {
      return {
        accountId: 'mockedAccountId',
        partition: 'mockedPartition',
        arn: 'mockedArn',
        userId: 'mockedUserId',
      };
  ...
  describe('constructor', () => {
    beforeEach(() => {
      plugin = new AssociateWafPlugin(serverless, options, v3Utils)
    })
  ...
  describe('without configuration', () => {
    ...
    describe('with invalid property "custom.associateWaf.version"', () => {
      ...
      it('should log info when invalid version configuration exists ', () => {
        expect(v3Utils.log.warning).toHaveBeenLastCalledWith(expect.stringContaining('Invalid WAF Version Configuration'))
      })
  ...
  describe('associateWaf()', () => {
    ...
    it('should log info when unable to find REST API ID from CloudFormation stack', async () => {
      ...
      expect(v3Utils.log.warning).toHaveBeenLastCalledWith(expect.stringContaining('Unable to determine REST API ID'))
    })
    ...
    it('should log info when unable to find WAF', async () => {
      ...
      expect(v3Utils.log.warning).toHaveBeenLastCalledWith(expect.stringContaining('Unable to find WAF named'))
    })

    it('should log error when exception caught and throw serverlesserror', async () => {
      spy = await setupAwsErrorMessage(plugin)
      await expect(plugin.associateWaf())
      .rejects
      .toThrow(serverless.classes.Error)
      expect(v3Utils.log.error).toHaveBeenLastCalledWith(expect.stringContaining(AWSErrorMessage))
    })
    ...
    it('should associate WAF', async () => {
      ...
      expect(v3Utils.log.notice).toHaveBeenLastCalledWith(expect.stringContaining('Associating WAF...'))
    })
  ...
  describe('disassociateWaf()', () => {
    ...
    it('should log error when exception caught and throw serverlesserror', async () => {
      spy = await setupAwsErrorMessage(plugin)
      await expect(plugin.disassociateWaf())
      .rejects
      .toThrow(serverless.classes.Error)
      expect(v3Utils.log.error).toHaveBeenLastCalledWith(expect.stringContaining(AWSErrorMessage))
    })
    ...
    it('should disassociate WAF if associated', async () => {
      ...
      expect(v3Utils.log.notice).toHaveBeenLastCalledWith(expect.stringContaining('Disassociating WAF...'))
    })
    ...
    it('should not disassociate WAF if associated', async () => {
      ...
      expect(v3Utils.log.notice).not.toHaveBeenLastCalledWith(expect.stringContaining('Disassociating WAF...'))
    })

  })

  async function setupAwsErrorMessage(plugin){
    const spy = jest.spyOn(v3Utils.log, 'error')
    plugin.provider.request
